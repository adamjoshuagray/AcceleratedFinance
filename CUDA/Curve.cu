...
cudaError_t af_TimeCurveCopyToDevice(afTimeCurve_t* src, afTimeCurve_t** dst) {
  ...
  if (host_pairs == (afTimeCurvePair_t*) src + sizeof(afTimeCurve_t)) {
    ...
    result      = cudaMemcpy((char*)dev_mem + sizeof(afTimeCurve_t), src->pairs, src->count * sizeof(afTimeCurvePair_t), cudaMemcpyHostToDevice);
    if (result != cudaSuccess) {
...
//
// FIXME: This function is longer than I would like.
//
__host__
cudaError_t af_TimeCurveCopyToHost(afTimeCurve_t* src, afTimeCurve_t** dst, bool assume_contiguous, int count) {
  void* host_mem;
  cudaError_t result;
  // We want to make sure that we don't try and copy less than we should.
  if (assume_contiguous && count < 0) {
  ...
  if (host_curve->pairs == (afTimeCurvePair_t*) ((char*)src + sizeof(afTimeCurve_t)) && host_curve->count == count) {
    // Copy worked!
    host_curve->pairs = (afTimeCurvePair_t*) host_curve + sizeof(afTimeCurve_t);
    (*dst) = host_curve;
    return result;
  } else if (host_curve->count < 0) {
    // We seem to have copied the incorrect thing...
    return cudaErrorUnknown;
  } else if (host_curve->pairs == (afTimeCurvePair_t*) ((char*)src + sizeof(afTimeCurve_t)) && host_curve->count < count ) {
    // We over allocated on the host. Lets deallocate some memory.
    afTimeCurve_t* new_host_curve = (afTimeCurve_t*) realloc(host_curve, sizeof(afTimeCurve_t) + host_curve->count * sizeof(afTimeCurvePair_t));
    if (new_host_curve == NULL) {
      // Try and free host allocated memory.
      free(host_mem);
      return cudaErrorUnknown;
    }
    host_curve->pairs = (afTimeCurvePair_t*) ((char*)host_curve + sizeof(afTimeCurve_t));
    (*dst) = new_host_curve;
    return result;
  } else if (host_curve->pairs == (afTimeCurvePair_t*) ((char*)src + sizeof(afTimeCurve_t)) && host_curve->count > count) {
    ...
    afTimeCurvePair_t* dev_start_ptr = (afTimeCurvePair_t*) ((char*)src + count * sizeof(afTimeCurvePair_t));
    int length = new_host_curve->count - count;
    // Use void* here
    afTimeCurvePair_t* host_start_ptr = (afTimeCurvePair_t*) ((char*)new_host_curve + count * sizeof(afTimeCurvePair_t));
    result = cudaMemcpy(host_start_ptr, dev_start_ptr, length * sizeof(afTimeCurvePair_t), cudaMemcpyDeviceToHost);
    if (result != cudaSuccess) {
      free(new_host_curve);
      return result;
    }
    // Fix up the pointer.
    new_host_curve->pairs = (afTimeCurvePair_t*) ((char*)new_host_curve + sizeof(afTimeCurve_t));
    // Return the result.
    (*dst) = new_host_curve;
    return result;
  } else {
    ...
    afTimeCurvePair_t* host_start_ptr = (afTimeCurvePair_t*) ((char*)new_host_curve + sizeof(afTimeCurve_t));
    result = cudaMemcpy(host_start_ptr, new_host_curve->pairs, new_host_curve->count * sizeof(afTimeCurvePair_t), cudaMemcpyDeviceToHost);
    if (result != cudaSuccess) {
      free(new_host_curve);
      return cudaErrorUnknown;
    }
